...
## 🔬 Practical Projects
* [**Azure Sentinel SOC Detection Lab with Entra ID Least-Privilege Automation**](https://github.com/safayetchowdhury/Azure-Sentinel-KQL-Entra-ID-Threat-Detection-Lab): Built and validated a cloud native SOC detection pipeline on Azure Sentinel, developing 3 custom KQL rules against 3 distinct attack techniques including port scanning, SSH brute force, and volumetric flooding. One rule additionally confirmed detecting live, unsimulated scanning traffic from the public internet during testing. Configured a least-privilege Entra ID service identity to ensure automation could never operate with more access than required.
