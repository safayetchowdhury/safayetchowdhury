...
## 🔬 Practical Projects
* **AutoSOC. Azure Sentinel plus Claude Triage and Auto Remediation (In Progress)**: Designing and building a cloud native SOC pipeline on **Azure Sentinel** that detects simulated network attacks including port scans, brute force attempts, and SYN floods through custom KQL rules, uses Claude API to correlate and triage alerts into
*MITRE ATT&CK* tagged incidents, and auto remediates high risk threats through Azure NSG, reducing alert noise
and manual analyst workload. Currently finalizing detection tuning and remediation logic and targeting completion
soon.
